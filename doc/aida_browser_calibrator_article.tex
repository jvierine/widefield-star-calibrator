...
features, attempts a blind quad asterism match, seeds the selected optical
...
Fourth, the code builds image quads and catalog quads. As in astrometry.net,
the widest pair in each four-star group defines a normalized coordinate system
and the two remaining stars define a four-dimensional code
\((C_x,C_y,D_x,D_y)\). Nearby quad-code pairs propose rotations. The score is
based on how many catalog stars are explained by the same rotation, the angular
residuals, ambiguity checks, and conflicts. Later projected stages are guarded
by pixel-space RMS so that bad additions can be rejected.
...
with approximately correct centroids, the quad matcher is quite robust. If
...
\subsection{Quad Signature}

For each set of candidate points, the blind matcher builds four-star
asterisms. The most widely separated pair is labeled \(A,B\) and defines a
local coordinate system. The remaining stars are labeled \(C,D\), sorted by
their normalized coordinate along the \(A\rightarrow B\) axis. The current
signature is the scale-free descriptor
\begin{equation}
  \mathbf{q} =
  \begin{bmatrix}
    C_x\\ C_y\\ D_x\\ D_y
  \end{bmatrix}.
\end{equation}
Degenerate quads are rejected before indexing. The four vertices must be
distinct detections or catalog stars, so the same star candidate cannot appear
twice in one quad. For image-detection quads, the shortest pixel side must also
be at least
\begin{equation}
...
\begin{equation}
  d_{\max} \le \frac{W}{4}.
\end{equation}
This prevents very large quads from dominating the search. Such quads are a
poor choice for wide lenses because small radial-model errors near the edge of
the field produce large changes in their normalized coordinates. After these
pixel-scale checks, the matcher still requires enough area to avoid nearly
collinear quadruples, which are poor rotation constraints and common in noisy
detections.

Catalog quad signatures are inserted into a four-dimensional KD-tree. For each
image-detection quad, the matcher searches the KD-tree for nearby catalog
signatures. A nearby signature does not prove an identification; it only
proposes a candidate correspondence between four image detections and four
catalog stars. Each proposed correspondence gives a rotation hypothesis. The
rotation is estimated from the \(A,B\) vector pair, checked against \(C,D\),
and then scored against all available stars, not just the four that generated
the hypothesis.
...
\begin{enumerate}
  ...
  \item \textbf{Blind spherical bootstrap.} Candidate image detections are
        ...
        unit vectors. Four-star quad asterisms from the image are matched against
        local catalog quads. Some blind-search modes can request regional
  ...
  \item \textbf{Whole-field verification.} A matching quad is only a proposal.
        The candidate rotation is accepted only if it explains additional
        detections across the image with small angular residuals and without
        excessive catalog or detection conflicts.
...
ordinary phone-camera images is not primarily a better asterism matcher, but a
...
dominate the asterism search space and lead the blind matcher down incorrect
...
Possible improvements include explicit indexing by angular scale, better use of
magnitude ordering, adaptive pre-flattening grids for fisheye lenses, and a
staged local-to-global verification procedure closer to astrometry.net.
