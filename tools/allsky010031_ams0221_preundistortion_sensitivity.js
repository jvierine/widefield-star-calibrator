...
const TEST_CASE = {
    ...
    image: IMAGE_NAME,
    imagePath: path.join(ROOT, "calibration_images", IMAGE_NAME),
    width: 1920,
    height: 1080,
...
function defaultOptmod2Optpar(testCase) {
    return [1.0, testCase.width / Math.max(1, testCase.height), 0, 0, 0, 0, 0, 0.7];
}

function perturbParameter(optpar, spec, delta) {
    const out = cloneOptpar(optpar);
    out[spec.index] = spec.mode === "relative" ? optpar[spec.index] * (1 + delta) : optpar[spec.index] + delta;
    return out;
}
...
function deltaForValue(optpar, spec, value) {
    return spec.mode === "relative" ? value / optpar[spec.index] - 1 : value - optpar[spec.index];
}
...
function undistortOptmod2Point(point, testCase, optpar) {
    const f1 = optpar[0];
    const f2 = optpar[1];
    const du = optpar[5];
    const dv = optpar[6];
    const radialAlpha = optpar[7];
    const x = (((point.x + 1) / testCase.width) - 0.5 - du) / f1;
    const y = (((point.y + 1) / testCase.height) - 0.5 - dv) / f2;
    const r = Math.hypot(x, y);
    const scale = Math.min(testCase.width, testCase.height) * 0.32;
    if (r < 1e-12 || Math.abs(radialAlpha) < 1e-9) {
        return {x: testCase.width / 2, y: testCase.height / 2, planeX: 0, planeY: 0};
    }
    const theta = Math.asin(Math.max(-0.999999, Math.min(0.999999, r))) / radialAlpha;
    const planeR = Math.tan(theta);
    const planeX = x / r * planeR;
    const planeY = y / r * planeR;
    return {
        x: testCase.width / 2 + planeX * scale,
        y: testCase.height / 2 + planeY * scale,
        planeX,
        planeY,
    };
}

function undistortCollection(points, testCase, optpar) {
    return points.map(point => {
        const undistorted = undistortOptmod2Point(point, testCase, optpar);
        return {...point, rawX: point.x, rawY: point.y, ...undistorted};
    });
}
...
function evaluateAsterism(testCase, rawDetections, catalogRaw, validation, undistortOptpar) {
    ...
    const identification = AutoIdentifier.identifyStarsByAsterisms(catalogUndistorted, undistortedDetections, {
        ...
        maxCatalogStars: Math.max(200, Math.min(420, catalogUndistorted.length)),
        ...MATCHER_OPTIONS,
    });
    return {
        ...scoreIdentificationByDetectionId(identification.matches, validation),
...
function sortedUnique(points) {
    const seen = new Set();
    return points
        .filter(point => Number.isFinite(point.delta) && Number.isFinite(point.value))
        .sort((a, b) => a.delta - b.delta)
        .filter(point => {
            const key = `${point.kind || "grid"}:${point.delta.toPrecision(12)}`;
            if (seen.has(key)) {
                return false;
            }
            seen.add(key);
...
async function buildSensitivityData() {
    const imageData = readPngImageData(TEST_CASE.imagePath);
    const detectionResult = await StarDetector.detectBrightStars(imageData, DETECTOR_OPTIONS);
    const catalogRaw = projectStars(TEST_CASE, TEST_CASE.optpar, MAX_MAG);
    const validation = knownLensValidationMap(detectionResult.detections, catalogRaw, TEST_CASE.matchRadiusPx);
    const defaultOptpar = defaultOptmod2Optpar(TEST_CASE);
    const baseline = evaluateAsterism(TEST_CASE, detectionResult.detections, catalogRaw, validation, TEST_CASE.optpar);
    process.stderr.write("evaluating optmod 2 default optpar\n");
    const defaultCase = evaluateAsterism(TEST_CASE, detectionResult.detections, catalogRaw, validation, defaultOptpar);
    const sweeps = [];
    for (const spec of PARAMETERS) {
        ...
        const grid = linspace(-spec.span, spec.span, SWEEP_POINTS).map(delta => ({
            kind: "grid",
            delta,
            value: spec.mode === "relative" ? TEST_CASE.optpar[spec.index] * (1 + delta) : TEST_CASE.optpar[spec.index] + delta,
            ...evaluateAsterism(TEST_CASE, detectionResult.detections, catalogRaw, validation,
                perturbParameter(TEST_CASE.optpar, spec, delta)),
        }));
        const defaultValue = defaultOptpar[spec.index];
        const defaultDelta = deltaForValue(TEST_CASE.optpar, spec, defaultValue);
        const defaultPoint = {
            kind: "default",
            delta: defaultDelta,
            value: defaultValue,
            ...evaluateAsterism(TEST_CASE, detectionResult.detections, catalogRaw, validation,
                perturbParameter(TEST_CASE.optpar, spec, defaultDelta)),
        };
        sweeps.push({...spec, points: sortedUnique(grid.concat([defaultPoint]))});
    }
    ...
    return {
        testCase: {
            id: TEST_CASE.id,
            image: path.relative(ROOT, TEST_CASE.imagePath),
            width: TEST_CASE.width,
            height: TEST_CASE.height,
            optmod: TEST_CASE.optmod,
        },
        maxMag: MAX_MAG,
...
function makeRecommendation(data) {
    ...
    return {
        ...
        defaultCorrect: data.defaultCase.correct,
        zoomTolerance90Percent: `${fmt(zoomLo, 2)}x to ${fmt(zoomHi, 2)}x`,
        strategy: [
            "For allsky7/optmod 2, pixel pre-undistortion is mainly sensitive to focal scale, principal point, and the radial alpha parameter.",
            "The three attitude angles are shown for completeness, but this pre-undistortion diagnostic does not use attitude; those plots should be flat.",
            "Use the default optmod 2 vector as a plausible first flattening candidate, but keep the calibrated f1/f2 aspect and radial alpha in the search grid.",
            "A common f1/f2 zoom sweep is useful because triangle ratios are partly scale-invariant while pixel-side gates still introduce scale dependence.",
        ],
        parameterTolerance90Percent: data.sweeps.map(sweep => ({name: sweep.label, tolerance: toleranceText(sweep, baseline)})),
    };
}

function actualValueForDelta(sweep, delta) {
    if (sweep.name === "zoom") {
        return 1 + delta;
    }
    const zero = sweep.points.find(point => Math.abs(point.delta) < 1e-10);
    const trueValue = zero ? zero.value : NaN;
    if (!Number.isFinite(trueValue)) {
        return NaN;
    }
    return sweep.mode === "relative" ? trueValue * (1 + delta) : trueValue + delta;
}
...
function plotSvg(sweep, baselineCorrect) {
    ...
    const pointSvg = sweep.points.map(point => {
        ...
        const title = point.kind === "default" ?
            `${sweep.label} default ${fmt(point.value)}: ${point.correct} correct` :
            `${sweep.label} ${fmt(point.value)}: ${point.correct} correct`;
        return `<circle class="${cls}" cx="${fmt(sx(point.delta), 1)}" cy="${fmt(sy(point.correct), 1)}" r="${point.kind === "default" ? 6 : 4}"><title>${escapeHtml(title)}</title></circle>`;
    }).join("\n");
    const ticks = [minX, 0, maxX]
        .filter((value, index, arr) => value >= minX && value <= maxX &&
    ...
    const topTicks = ticks.map(value =>
        `<text class="tick top-tick" x="${fmt(sx(value), 1)}" y="${pad - 12}" text-anchor="middle">${fmt(actualValueForDelta(sweep, value), 3)}</text>`
    ).join("\n");
    ...
    const bottomLabel = sweep.name === "zoom" ? "zoom factor minus one" :
    ...
    return `<svg class="plot" viewBox="0 0 ${w} ${h}">
        ...
        <text class="axis" x="${w / 2}" y="${h - 10}" text-anchor="middle">${escapeHtml(bottomLabel)}</text>
        <text class="axis" x="14" y="${h / 2}" text-anchor="middle" transform="rotate(-90 14 ${h / 2})">correct matches</text>
        ${bottomTicks}${topTicks}
...
function processingStepsHtml(data) {
    const matcherOptions = {
        ...
        maxCatalogStars: "clamped to 200..420 from the perturbed catalog",
    };
    return `<section class="note">
...
<ol>
<li>Load the allsky7 PNG image and run the automatic star detector once with the detector options below.</li>
<li>Project the Yale catalogue through the supplied calibrated optmod 2 optpar. This creates the reference catalogue positions for the test case.</li>
...
<li>Pre-undistort both the oracle-validated detections and the catalogue positions with the perturbed optmod 2 vector for that sweep point.</li>
...
function pageHtml(data) {
    ...
    const plots = data.sweeps.concat([data.zoomSweep]).map(sweep => `<section class="card">${plotSvg(sweep, data.baseline.correct)}
...
<head>
<meta charset="utf-8">
<title>Allsky 010031 AMS0221 Optmod 2 Sensitivity</title>
<style>
...
<p>Gold markers show the app's default optmod 2 parameter value for each one-parameter sweep.</p>
...
async function main() {
    ...
    console.log(`default optmod 2 correct matches: ${data.defaultCase.correct}`);
